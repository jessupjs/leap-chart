...
@Component({
  selector: 'app-scatter-view-brush',
  templateUrl: './scatter-view-brush.component.html',
  styleUrls: ['./scatter-view-brush.component.scss']
})
export class ScatterViewBrushComponent implements OnInit {
   ...
   * colorTarget
   */
  colorTarget(cat, el): void {
  	const vis = this;
    this.child.els.bubblesG.selectAll(el)
      .each(function(d, i) {
        if (d.name === cat) {
          	d3.select(this)
    ...
    function createCluster(pos, range, rands, cat) {
      for (let i = 0; i < rands; i++) {
        ...
        const obj = {
          ...
          r: Math.random() * 2 * vis.dataConfigs.inputR[1],
        }
      ...
      vis.child.els.bubblesG.selectAll('.bubble')
        .each(function(d) {
			...
			if (dist <= r) {
				...
				d3.select(this).classed('bubble', d => {
			    	return false;
				})

	          	d3.select(this).classed('bubble-select', d => {
	                return true;
	            })
	        }
        })
