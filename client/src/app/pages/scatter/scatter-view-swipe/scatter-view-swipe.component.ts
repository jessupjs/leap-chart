...
export class ScatterViewSwipeComponent implements OnInit {
  ...
  getData(): any[] {
    ...
    const range2 = [7.5, 20];
    const rands2 = 75;
    createCluster(pos2, range2, rands2, 'cat0');
    ...
    const range5 = [5, 7.5];
    const rands5 = 25;
    createCluster(pos5, range5, rands5, 'cat2');
    ...
    const range7 = [7.5, 10];
    const rands7 = 38;
    createCluster(pos7, range7, rands7, 'cat4');

    const pos8 = [40, 60];
    const range8 = [7.5, 50];
    const rands8 = 68;
    createCluster(pos8, range8, rands8, 'cat5');
  ...
  onFrame(frame: any): void {
    ...
    if (frame.fingers.length > 0) {
      ...
      this.moveElements(vis.nameHovered, pointerDirection)
    }
  }
  ...
  */
   ...
   * move Elements
   */
  moveElements(cat = 'cat1', direction): void {
    ...
    if (cat !== '') {
      dir = (direction === 'left direction') ? catDir[cat][0] : catDir[cat][1];
    }
      ...
      .each(function(d, i) {
        ...
        if (d.name === cat) {
          d3.select(this)
            ...
            .transition()
            .duration(200)
            .attr('cx', d => vis.child.tools.scX(d.x + dir * 3))
            // .attr('cy', d => vis.child.tools.scY(d.y + dir * 3))
